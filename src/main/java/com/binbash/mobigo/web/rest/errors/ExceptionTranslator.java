...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleAnyException(Throwable ex, NativeWebRequest request) {
        ...
        if (pdCause.getStatus() >= 500) {
            LOG.error("Internal server error on {} :", getPathValue(request), ex);
        } else {
    ...
    private ProblemDetailWithCause getProblemDetailWithCause(Throwable ex) {
        if (
            ex instanceof com.binbash.mobigo.service.UsernameAlreadyUsedException
        ) return (ProblemDetailWithCause) new LoginAlreadyUsedException().getBody();
        if (
            ex instanceof com.binbash.mobigo.service.EmailAlreadyUsedException
        ) return (ProblemDetailWithCause) new EmailAlreadyUsedException().getBody();
        if (
            ex instanceof com.binbash.mobigo.service.InvalidPasswordException
    ...
    private boolean containsPackageName(String message) {
        ...
        return StringUtils.containsAny(message, "org.", "java.", "net.", "jakarta.", "javax.", "com.", "io.", "de.", "com.binbash.mobigo");
    }
}
