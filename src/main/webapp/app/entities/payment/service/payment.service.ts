...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'datePaiement'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      datePaiement: payment.datePaiement?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      datePaiement: restPayment.datePaiement ? dayjs(restPayment.datePaiement) : undefined,
    };
  }
