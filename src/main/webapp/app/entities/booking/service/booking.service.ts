...
type RestOf<T extends IBooking | NewBooking> = Omit<T, 'dateReservation'> & {
...
export class BookingService {
  ...
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/bookings/_search');
  ...
  search(req: Search): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestBooking[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),

      catchError(() => scheduled([new HttpResponse<IBooking[]>()], asapScheduler)),
    );
  }
  ...
  protected convertDateFromClient<T extends IBooking | NewBooking | PartialUpdateBooking>(booking: T): RestOf<T> {
    return {
      ...booking,
      dateReservation: booking.dateReservation?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBooking: RestBooking): IBooking {
    return {
      ...restBooking,
      dateReservation: restBooking.dateReservation ? dayjs(restBooking.dateReservation) : undefined,
    };
  }
