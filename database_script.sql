...
ALTER TABLE payment DROP CONSTRAINT IF EXISTS payment_statut_check;
ALTER TABLE payment
  ADD CONSTRAINT payment_statut_check CHECK (
    statut::text = ANY
  (ARRAY['EN_ATTENTE','EN_COURS','COLLECTE_REUSSIE','REUSSI','ECHOUE','REMBOURSE']::text[]) );

ALTER TABLE notification DROP CONSTRAINT IF EXISTS notification_type_check;
ALTER TABLE notification
  ADD CONSTRAINT notification_type_check CHECK (
    type::text = ANY (ARRAY[
  'BOOKING_RECEIVED','BOOKING_ACCEPTED','BOOKING_REJECTED','BOOKING_CANCELLED',
  'TRIP_MODIFIED','TRIP_CANCELLED','TRIP_REMINDER','RATING_RECEIVED','MESSAGE_RECEIVED',
